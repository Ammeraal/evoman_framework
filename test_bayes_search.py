...
class EvoEAEstimator(BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        for i in range(load_generation + 1, generations):
            ...
            offspring = self.one_point_crossover(selected_parents, pop_size, self.elitism)
            pop = self.mutate(offspring, self.mut_rate, mean, sigma)
            np.append(pop,selected_parents[np.argsort(g.fitness for g in selected_parents)][0:self.elitism])
            # saving system
